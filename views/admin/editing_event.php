...
<p><font color="#FF0000"><blink><?=@$info?></blink></font></p>
		...
		<td>
		<?=form_open()?>
		<p align="center">Add your event announcement here in this form and then just submit.</p>
		...
		<?endforeach?>
		<p align="center"><?=form_submit('add_event', 'Add News')?></p>
		<?=form_close()?>
		...
		<td width="7%">Delete</td>
			...
			<?foreach($news->result() as $t):?>
				...
				<td><?=$t->Subject?></td>
				<td><?=$t->HTML?></td>
				<td><?=anchor('admin/event_edit/'.$t->Bil, 'EDIT', 'title = "EDIT NEWS '.$t->Bil.'"')?></td>
				<td><?=anchor('admin/event_del/'.$t->Bil, 'DELETE', 'title = "DELETE NEWS '.$t->Bil.'"')?></td>
				</tr>
			<?endforeach?>
		</table>
		</td>
